...
const Profile = {
  ...
  oneProfile: async (profileId) => {
    ...
    try {
      const [dependentsResults] = await db.query(dependentsQuery, [profileId]);
      const [accountsResults] = await db.query(accQuery, [profileId]);
      const [invoicesResults] = await db.query(invQuery, [profileId]);
      const [profileResults] = await db.query(profQuery, [profileId]);
